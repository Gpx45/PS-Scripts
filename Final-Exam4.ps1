...
$server_array = "DC1", "DC2", "DC3", "RouterA", "ADFS", "WebApp"
...
$server_array = "DC4", "DC5", "CA", "RouterB", "Client"
...
$server_array = "DC1", "DC2", "DC3", "DC4", "DC5", "CA", "RouterA", "RouterB", "ADFS", "WebApp", "Client"
...
function Configure-Machine() {
    ...
    Invoke-Command -VMName WebApp -Credential $localAuth -ScriptBlock {
        New-NetIPAddress -IPAddress 172.16.0.5 -PrefixLength 21 -InterfaceAlias Ethernet -DefaultGateway 172.16.0.1
        Set-DnsClientServerAddress -ServerAddresses 172.16.0.2 -InterfaceAlias *
        #Install-WindowsFeature -Name web-server,web-asp-net,web-management-console -IncludeManagementTools -Restart
        Rename-Computer -NewName webapp -Restart
    }


    Invoke-Command -VMName ADFS -Credential $localAuth -ScriptBlock {
        New-NetIPAddress -IPAddress 172.16.0.6 -PrefixLength 21 -InterfaceAlias Ethernet -DefaultGateway 172.16.0.1
        Set-DnsClientServerAddress -ServerAddresses 172.16.0.2 -InterfaceAlias *
        #Install-WindowsFeature -Name web-server,web-asp-net,web-management-console -IncludeManagementTools -Restart
        Rename-Computer -NewName cs03-adfs -Restart
    }
...
#------------------------------------------------------------------------------------------------------------------------
    ...
    Invoke-Command -VMName Client -Credential $localAuth -ScriptBlock {
        New-NetIPAddress -IPAddress 172.16.8.5 -PrefixLength 21 -InterfaceAlias Ethernet -DefaultGateway 172.16.8.1
        Set-DnsClientServerAddress -ServerAddresses 172.16.8.2 -InterfaceAlias *
        #Install-WindowsFeature -Name Adcs-Cert-Authority -IncludeManagementTools -Restart
        Rename-Computer -NewName cs03-client -Restart
    }
...
function Setup-Machines(){
    ...
    Read-Host "Wait for DC5 to complete"
     ...
     Invoke-Command -VMName DC2 -Credential $localAuth -ArgumentList $pwd, $domainAuth -ScriptBlock {
        param($pwd, $domainAuth)
        Install-AddsDomainController -DomainName child.contoso.com -SafeModeAdministratorPassword $pwd -Credential (Get-Credential child\administrator) -NoDnsOnNetwork -NoGlobalCatalog -MoveInfrastructureOperationMasterRoleIfNecessary -Force
...
$cert = New-SelfSignedCertificate -certstorelocation cert:\localmachine\my -dnsname webapp.fake.com -FriendlyName 'My Web App Certificate'
$pwd = ConvertTo-SecureString Pa11word -Force -AsPlainText
$path = 'cert:\localMachine\my\' + $cert.thumbprint
Write-Host $path
Export-PfxCertificate -cert $path -FilePath c:\cert.pfx -Password $pwdhttps://bconline.broward.edu/d2l/le/content/360663/viewContent/9543057/View?ou=360663
#>
